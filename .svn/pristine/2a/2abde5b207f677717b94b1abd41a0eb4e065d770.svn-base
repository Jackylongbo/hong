...
-(void)InitCell
{
    ...
    progressView = [[ProgressBarView alloc] initWithFrame:CGRectMake(self.contentView.bounds.size.width-80, 30, 50, 50)];
    ...
    progressView.progressBarArcWidth = 6.0f;
    ...
    progressView.progressBarShadowOpacity = .1f;
    progressView.duration = 0.001f;//调整动画的播放速度
    progressView.currentProgress = 0.0f;
    [progressView run:0.0f];
    
    progressLabel = [[UILabel alloc] initWithFrame:CGRectMake((progressView.frame.size.width-40)/2, (progressView.frame.size.height-20)/2, 40, 20)];
...
- (void)animationDidStop:(CAAnimation *)processAnimation finished:(BOOL)flag {
    ...
    if (progressView.currentProgress < 1.0) {
        progressLabel.text = [NSString stringWithFormat:@"%ld%@", stringInt, @"%"];
