...
- (void)viewDidLoad {
    ...
    m_scrollView=[self create_scrollView];//创建一个scrollview
    pageCtrl=[self create_pageControl];//创建一个pagecontrol
...
{
    UIScrollView *scrollview=[[UIScrollView alloc]initWithFrame:CGRectMake(0, m_size.height/5, m_size.width, m_size.height)];
...
{
    UIPageControl* pageControl=[[UIPageControl alloc]initWithFrame:CGRectMake(m_size.width/4, m_size.height*0.8, m_size.width/2, 30)];
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (pageCtrl.currentPage>=[imageArray count]) {
